...
if not exist ".venv\\Scripts\\python.exe" (
    %PY_CMD% -m venv .venv >> "%LOG_PATH%" 2>&1
    if errorlevel 1 (
...
%VENV_PY% -m pip install --upgrade pip setuptools wheel >> "%LOG_PATH%" 2>&1
if errorlevel 1 (
...
if exist "%REQ_FILE%" (
    %VENV_PY% -m pip install -r "%REQ_FILE%" >> "%LOG_PATH%" 2>&1
) else (
    %VENV_PY% -m pip install -r requirements-client.txt >> "%LOG_PATH%" 2>&1
)
if errorlevel 1 (
...
%VENV_PY% -m pip install pyinstaller >> "%LOG_PATH%" 2>&1
if errorlevel 1 (
...
%VENV_PY% -m PyInstaller --onefile --name RemoteControllerClient --clean --noconsole --log-level WARN ^
    --collect-all av ^
    --collect-all aiortc ^
